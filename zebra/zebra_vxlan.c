...
static void zvni_print(zebra_vni_t *zvni, void **ctxt)
{
	...
	if (json == NULL) {
		...
		vty_out(vty, " Advertise-svi-macip: %s\n",
			zvni->advertise_svi_macip ? "Yes" : "No");
	}
}
...
static int zvni_neigh_install(zebra_vni_t *zvni, zebra_neigh_t *n)
{
	...
	ZEBRA_NEIGH_SET_ACTIVE(n);
	return kernel_add_neigh(vlan_if, &n->ip, &n->emac, flags);
}
			     ...
			     struct ipaddr *ip)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("%u:SVI %s(%u) VNI %u, sending GW MAC %s IP %s flags 0x%x del to BGP",
			   ifp->vrf_id, ifp->name,
...
static void zvni_build_hash_table()
{
	...
	for (rn = route_top(zns->if_table); rn; rn = route_next(rn)) {
		...
		if (zl3vni) {
			...
			/* VNI hash entry is expected to exist,
			 * if the BGP process is killed
			 */
			...
			if (zvni) {
				...
				/* Send Local Neighbor entries to client */
			     ...
			     zebra_neigh_t *n)
{
	...
	return kernel_add_neigh(zl3vni->svi_if, &n->ip, &n->emac, flags);
}
				     ...
				     struct in_addr vtep_ip)
{
	...
	if (n && !mac) {
		zlog_debug("Failed to locate MAC %s for neigh %s VNI %u upon remote MACIP DEL",
			  prefix_mac2str(macaddr, buf, sizeof(buf)),
				   ...
				   bool use_json)
{
	...
	hash_iterate(
		zvrf->vni_table,
		(void (*)(struct hash_backet *, void *))zvni_print_hash_detail,
		&zes);
...
int zebra_vxlan_dup_addr_detection(struct zserv *client, uint8_t length,
				   struct zebra_vrf *zvrf)
{
	struct stream *s;
	int time = 0;
					...
					struct ipaddr *ip)
{
	...
	zvrf = vrf_info_lookup(zvni->vxlan_if->vrf_id);
	if (!zvrf) {
		zlog_debug("%s: VNI %u vrf lookup failed.", __PRETTY_FUNCTION__,
			   zvni->vni);
		return -1;
	}
 ...
 * receive direction.
 */
void zebra_vxlan_flood_control(struct zserv *client,
 ...
 * routes
 */
int zebra_vxlan_advertise_svi_macip(struct zserv *client, u_short length,
				    struct zebra_vrf *zvrf)
{
	struct stream *s;
	int advertise;
	...
	if (zvrf_id(zvrf) != VRF_DEFAULT) {
		zlog_debug("EVPN SVI macip Adv for non-default VRF %u",
			   zvrf_id(zvrf));
		return -1;
	}
...
static void zvni_send_mac_hash_entry_to_client(struct hash_backet *bucket,
					       void *arg)
{
	struct mac_walk_ctx *wctx = arg;
...
static void zvni_send_neigh_hash_entry_to_client(struct hash_backet *bucket,
						 void *arg)
{
	struct mac_walk_ctx *wctx = arg;
	...
	if (CHECK_FLAG(zn->flags, ZEBRA_NEIGH_LOCAL) &&
		...
		zvni_neigh_send_add_to_client(wctx->zvni->vni,
					      &zn->ip,
					      &zn->emac, zn->flags,
					      zn->loc_seq);
	}
...
static void zvni_evpn_cfg_cleanup(struct hash_backet *bucket, void *ctxt)
{
	zebra_vni_t *zvni = NULL;
...
int zebra_evpn_cfg_clean_up(struct zserv *client)
{
	struct vrf *vrf;
	struct zebra_vrf *zvrf;
