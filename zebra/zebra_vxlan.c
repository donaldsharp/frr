...
static void zebra_vxlan_rem_mac_del(zebra_vni_t *zvni, zebra_mac_t *zmac);
...
static void zvni_print_neigh_hdr(struct vty *vty, struct neigh_walk_ctx *wctx)
{
	...
	if (CHECK_FLAG(n->flags, ZEBRA_NEIGH_LOCAL)) {
		...
		if (json_vni == NULL) {
			vty_out(vty, "%*s %-6s %-5s %-8s %-17s %-30s %u/%u\n",
				...
				zvni_print_neigh_flags(n, flags_buf), state_str,
	...
	if (CHECK_FLAG(mac->flags, ZEBRA_MAC_LOCAL)) {
		...
		if (json_mac_hdr == NULL) {
			...
			vty_out(vty, "%-17s %-6s %-5s %-30s %-5s %u/%u\n", buf1,
				"remote", zvni_print_mac_flags(mac, flags_buf),
	...
	if (((wctx->flags & DEL_LOCAL_NEIGH) && (n->flags & ZEBRA_NEIGH_LOCAL))
		...
		if (wctx->upd_client && (n->flags & ZEBRA_NEIGH_LOCAL))
			zvni_neigh_send_del_to_client(
				wctx->zvni->vni, &n->ip, &n->emac, n->flags,
				n->state, false /*force*/);
					 ...
					 uint32_t flags, int state, bool force)
{
	...
	return zvni_macip_send_msg_to_client(vni, macaddr, ip, flags, 0, state,
			     ...
			     struct ipaddr *ip)
{
	...
	zvni_neigh_send_del_to_client(zvni->vni, &n->ip, &n->emac, n->flags,
				      ZEBRA_NEIGH_ACTIVE, false /*force*/);
...
static inline void
zvni_local_neigh_update_log(const char *pfx, zebra_neigh_t *n, bool is_router,
			    ...
			    bool inform_bgp, const char *sfx)
{
	...
	zlog_debug("%s neigh vni %u ip %s mac %s f 0x%x%s%s%s%s%s%s %s", pfx,
		   n->zvni->vni, ipaddr2str(&n->ip, ipbuf, sizeof(ipbuf)),
...
static int zvni_local_neigh_update(zebra_vni_t *zvni, struct interface *ifp,
				   ...
				   bool dp_static)
{
	...
	if (upd_mac_seq && zmac->loc_seq != mac_new_seq) {
		...
		zmac->loc_seq = mac_new_seq;
		if (zvni_mac_send_add_to_client(zvni->vni, macaddr, zmac->flags,
				       ...
				       struct zebra_evpn_es *es)
{
	...
	return zvni_macip_send_msg_to_client(vni, macaddr, NULL, flags, seq,
				...
				bool was_static)
{
	...
	res = dplane_rem_mac_add(zvni->vxlan_if, br_ifp, vid, &mac->macaddr,
...
{
	if (new_bgp_ready)
		zvni_mac_send_add_to_client(mac->zvni->vni, &mac->macaddr,
					    mac->flags, mac->loc_seq, mac->es);
	else if (old_bgp_ready)
		zvni_mac_send_del_to_client(mac->zvni->vni, &mac->macaddr,
...
static zebra_mac_t *
zebra_vxlan_proc_sync_mac_update(zebra_vni_t *zvni, struct ethaddr *macaddr,
				 ...
				 struct sync_mac_ip_ctx *ctx)
{
	...
	if (!mac) {
		...
		if (!zebra_vxlan_mac_is_bgp_seq_ok(zvni, mac, seq, ipa_len,
						   ipaddr)) {
			ctx->ignore_macip = true;
			return NULL;
		}
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_MAC)
		zlog_debug("sync-mac %s vni %u mac %s es %s seq %d f 0x%x%s%s",
			   ctx->mac_created ? "created" : "updated", zvni->vni,
			   prefix_mac2str(macaddr, macbuf, sizeof(macbuf)),
			   mac->es ? mac->es->esi_str : "-", mac->loc_seq,
	...
	if (seq_change || es_change || !old_local)
		zvni_process_neigh_on_local_mac_change(zvni, mac, seq_change,
						       es_change);
...
{
	if (new_bgp_ready)
		zvni_neigh_send_add_to_client(n->zvni->vni, &n->ip, &n->emac,
					      n->mac, n->flags, n->loc_seq);
	else if (old_bgp_ready)
		zvni_neigh_send_del_to_client(n->zvni->vni, &n->ip, &n->emac,
					      n->flags, n->state,
					      true /*force*/);
}
...
zebra_vxlan_proc_sync_neigh_update(zebra_vni_t *zvni, zebra_neigh_t *n,
				   uint16_t ipa_len, struct ipaddr *ipaddr,
				   uint8_t flags, uint32_t seq, esi_t *esi,
				   struct sync_mac_ip_ctx *ctx)
{
	...
	if (!n) {
		...
		n = zvni_neigh_add(zvni, ipaddr, &mac->macaddr, mac, n_flags);
		n->ifindex = ifindex;
		ZEBRA_NEIGH_SET_ACTIVE(n);
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
		zlog_debug(
			...
			created ? "created" : "updated", n->zvni->vni,
			ipaddr2str(&n->ip, ipbuf, sizeof(ipbuf)),
			prefix_mac2str(&n->emac, macbuf, sizeof(macbuf)),
...
static void
zebra_vxlan_process_sync_macip_add(zebra_vni_t *zvni, struct ethaddr *macaddr,
				   uint16_t ipa_len, struct ipaddr *ipaddr,
				   uint8_t flags, uint32_t seq, esi_t *esi)
{
	struct sync_mac_ip_ctx ctx;
	...
	if (ipa_len) {
		...
		if (n
		    && !zebra_vxlan_neigh_is_bgp_seq_ok(zvni, n, macaddr, seq))
			return;
	}

	memset(&ctx, 0, sizeof(ctx));
	ctx.mac = zebra_vxlan_proc_sync_mac_update(
		zvni, macaddr, ipa_len, ipaddr, flags, seq, esi, &ctx);
	if (ctx.ignore_macip || !ctx.mac || !ipa_len)
		return;

	zebra_vxlan_proc_sync_neigh_update(zvni, n, ipa_len, ipaddr, flags, seq,
					   esi, &ctx);
}
...
static void zebra_vxlan_rem_mac_del(zebra_vni_t *zvni, zebra_mac_t *mac)
{
					...
					struct ipaddr *ip)
{
	...
	if (!!CHECK_FLAG(nbr->flags, ZEBRA_NEIGH_LOCAL)) {
		zvni_neigh_send_add_to_client(zvni->vni, ip, &nbr->emac,
					      nbr->mac, nbr->flags,
					      nbr->loc_seq);
	} else if (!!CHECK_FLAG(nbr->flags, ZEBRA_NEIGH_REMOTE)) {
	...
	bool local_inactive, bool dp_static)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN || IS_ZEBRA_DEBUG_EVPN_MH_NEIGH)
		zlog_debug(
			...
			is_router ? "router " : "",
			local_inactive ? "local_inactive " : "", zvni->vni);
