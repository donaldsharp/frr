...
static int l3vni_rmac_nh_list_cmp(void *p1, void *p2)
{
	...
	return memcmp(vtep_ip1, vtep_ip2, sizeof(struct ipaddr));
}
...
static void l3vni_rmac_nh_list_nh_delete(zebra_l3vni_t *zl3vni,
					 zebra_mac_t *zrmac,
					 struct ipaddr *vtep_ip)
{
	...
	for (ALL_LIST_ELEMENTS(zrmac->nh_list, node, nnode, vtep)) {
		if (memcmp(vtep, vtep_ip, sizeof(struct ipaddr)) == 0)
			break;
	}
	...
	struct zebra_vxlan_vni *vnip, void *arg)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("Build vni table for vni %u for Intf %s",
			vni, ifp->name);
				  ...
				  const struct ipaddr *vtep_ip)
{
	...
	if (!zrmac) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug(
				"L3VNI %u Remote VTEP change(%s -> %s) for RMAC %s",
				zl3vni->vni,
				inet_ntoa(zrmac->fwd_info.r_vtep_ip),
				ipaddr2str(vtep_ip, buf1, sizeof(buf1)),
				prefix_mac2str(rmac, buf, sizeof(buf)));
			     ...
			     const struct prefix *host_prefix)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("SVD NH ADD refcnt (%u) for nexthop %s", nh->refcnt,
...
static int zl3vni_send_add_to_client(zebra_l3vni_t *zl3vni)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug(
			"Send L3_VNI_ADD %u VRF %s RMAC %s VRR %s local-ip %s filter %s l3svd %u to %s",
	...
	if (state & NUD_PERMANENT) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug(
				...
				ifp->name, ifp->ifindex, state, zevpn->vni);
		return 0;
	}
