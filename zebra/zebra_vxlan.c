...
static int zvni_neigh_send_add_to_client(vni_t vni, struct ipaddr *ip,
					 ...
					 zebra_mac_t *zmac, uint8_t neigh_flags,
					 uint32_t seq)
{
	uint8_t flags = 0;
				   ...
				   bool is_router)
{
	...
	if (!neigh_on_hold) {
		...
		return zvni_neigh_send_add_to_client(
			zvni->vni, ip, &n->emac, n->mac, n->flags, n->loc_seq);
	} else {
		if (IS_ZEBRA_DEBUG_VXLAN)
...
static void process_sync_macip_add(zebra_vni_t *zvni, struct ethaddr *macaddr,
				     ...
				     bool sticky)
{
	...
	if (inform_client) {
		if (zvni_mac_send_add_to_client(zvni->vni, macaddr, mac->flags,
						mac->loc_seq, mac->es))
			return -1;
	}
