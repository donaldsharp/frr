...
static uint32_t nhg_get_next_id(void)
{
	while (1) {
		...
		if (id_counter == ZEBRA_NHG_PROTO_LOWER) {
			...
			frrtrace(1, frr_zebra, zebra_nhg_id_counter_wrapped,
				 id_counter);
			id_counter = 0;
			continue;
		}
		...
		frr_each(nhg_connected_tree, &nhe->nhg_depends, rb_node_dep) {
			if (IS_ZEBRA_DEBUG_NHG_DETAIL)
				zlog_debug("%s: nhe %p (%pNG), dep %p (%pNG)",
					   ...
					   rb_node_dep->nhe);
			frrtrace(2, frr_zebra, zebra_nhg_dep, nhe->id,
				 rb_node_dep->nhe->id);
...
afi_t zebra_nhg_get_afi(const struct nexthop_group *nhg, afi_t route_afi)
{
	...
	if (!all_same)
		zlog_warn(
			"Not all Nexthops had equivalent AFIs; "
...
static void zebra_nhg_set_valid(struct nhg_hash_entry *nhe, bool valid)
{
	...
	frr_each (nhg_connected_tree, &nhe->nhg_dependents, rb_node_dep) {
		if (!valid) {
			/*
			 * Grab the first nexthop from the depending nexthop
			 * group then let's find the nexthop in that group that
			 * matches my individual nexthop and mark it as no
			 * longer ACTIVE
			 */
					  ...
					  int max_num)
{
	...
	frr_each(nhg_connected_tree, &nhe->nhg_depends, rb_node_dep) {
		...
		if (!zebra_nhg_depends_is_empty(depend)) {
			...
			i = zebra_nhg_nhe2grp_internal(grp, i, depend, nhe, max_num);
		} else {
			bool found;
...
void zebra_nhg_dplane_result(struct zebra_dplane_ctx *ctx)
{
	...
	frrtrace(4, frr_zebra, zebra_nhg_dplane_result, ctx, dplane_op2str(op),
		 id, dplane_res2str(status));
	...
	frr_each (nhg_connected_tree, &zif->nhg_dependents, rb_node_dep) {
		...
		if (nexthop_is_ifindex_type(nh) &&
			...
			frr_each_safe (nhg_connected_tree,
				...
				if (IS_ZEBRA_DEBUG_NHG)
					zlog_debug(
						"%s dependent nhe %pNG Setting Reinstall flag",
						__func__,
