...
static void zebra_evpn_stale_remote_neigh_add(struct zebra_evpn *zevpn,
					      const struct ipaddr *ip,
					      const struct ethaddr *macaddr,
					      bool is_router)
{
	...
	if (!zmac) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug(
				"EVPN-GR: zmac for MAC %pEA not found. L2VNI %u",
				macaddr, zevpn->vni);
		return;
	}
	...
	if (!n) {
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug(
				"EVPN-GR: Can't create neigh entry for IP %pIA MAC %pEA, L2VNI %u",
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug(
			"EVPN-GR: Added stale remote %sneigh entry IP %pIA MAC %pEA, L2VNI %u",
				   ...
				   uint16_t state, bool is_router)
{
	...
	if (!n) {
		if (zrouter.graceful_restart)
			zebra_evpn_stale_remote_neigh_add(zevpn, ip, macaddr,
							  is_router);
		return 0;
	}
