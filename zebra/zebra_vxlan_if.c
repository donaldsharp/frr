...
static void zebra_vxlan_if_vni_iterate_callback(struct hash_bucket *bucket, void *ctxt)
{
	struct zebra_vxlan_vni *vni;
	struct zebra_vxlan_if_ctx *ctx;
...
static int zebra_vxlan_if_del_vni(struct interface *ifp, struct zebra_vxlan_vni *vnip)
{
	vni_t vni;
...
static int zebra_vxlan_if_update_vni(struct interface *ifp,
        struct zebra_vxlan_vni *vnip, uint16_t chgflags)
{
	vni_t vni;
	struct zebra_if *zif = NULL;
...
static int zebra_vxlan_if_add_vni(struct interface *ifp, struct zebra_vxlan_vni *vnip)
{
	vni_t vni;
	struct zebra_if *zif = NULL;
	zebra_evpn_t *zevpn = NULL;
	...
	if (zl3vni) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN) {
			...
			inet_ntop(AF_INET, &vnip->mcast_grp,
...
static void zebra_vxlan_if_vni_entry_del(struct zebra_if *zif, struct zebra_vxlan_vni *vni)
{
	if (vni) {
...
static int zebra_vxlan_if_vni_entry_add(struct zebra_if *zif, struct zebra_vxlan_vni *vni)
{
	...
	struct zebra_vxlan_vni *vni, void *ctxt)
{
	...
	if ((hashcount(old_vni_table) == 0) ||
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug("vxlan %s adding vni(%d, %d)", zif->ifp->name,
	...
	if (old_vni->access_vlan != vni->access_vlan) {
		...
		zebra_evpn_vl_vxl_ref(vni->access_vlan, vni->vni, zif);
		zebra_vxlan_if_update_vni(zif->ifp, vni, ZEBRA_VXLIF_VLAN_CHANGE);
...
int zebra_vxlan_if_vni_table_destroy(struct zebra_if *zif)
{
	...
	if (vni_info->vni_table) {
		zebra_vxlan_if_vni_iterate(zif,
			zebra_vxlan_if_vni_entry_del_callback, NULL);
...
struct zebra_vxlan_vni * zebra_vxlan_if_vni_find(const struct zebra_if *zif, vni_t vni)
{
	...
	vni_tmp.vni = vni;
	vnip = (struct zebra_vxlan_vni *)hash_lookup(vni_info->vni_table, (void *)&vni_tmp);
	...
	int (*func)(struct zebra_if *zif, struct zebra_vxlan_vni *, void *), void *arg)
{
	...
	ctx.func = func;
	ctx.arg = arg;
	hash_iterate(vni_info->vni_table, zebra_vxlan_if_vni_iterate_callback, &ctx);
}
...
int zebra_vxlan_if_vni_table_add_update(struct interface *ifp, struct hash *vni_table)
{
	...
	zebra_vxlan_if_vni_iterate(zif, zebra_vxlan_if_add_update_vni, old_vni_table);
...
int zebra_vxlan_if_vni_mcast_group_update(struct interface *ifp,
	vni_t vni_id, struct in_addr *mcast_group)
{
	...
	return zebra_vxlan_if_update_vni(ifp, vni, ZEBRA_VXLIF_MCAST_GRP_CHANGE);
}
...
int zebra_vxlan_if_down(struct interface *ifp)
{
	...
	zebra_vxlan_if_vni_iterate(zif,
		zebra_vxlan_if_vni_entry_down_callback, NULL);
...
int zebra_vxlan_if_up(struct interface *ifp)
{
	...
	zebra_vxlan_if_vni_iterate(zif,
		zebra_vxlan_if_vni_entry_up_callback, NULL);
...
int zebra_vxlan_if_del(struct interface *ifp)
{
	...
	if (IS_ZEBRA_VXLAN_IF_VNI(zif)) {
		vni_info = VNI_INFO_FROM_ZEBRA_IF(zif);
		zebra_evpn_vl_vxl_deref(
			vni_info->vni.access_vlan, vni_info->vni.vni, zif);
...
int zebra_vxlan_if_update(struct interface *ifp, uint16_t chgflags)
{
	...
	zebra_vxlan_if_vni_iterate(zif,
		zebra_vxlan_if_vni_entry_update_callback, &chgflags);
...
int zebra_vxlan_if_add(struct interface *ifp)
{
	...
	if (IS_ZEBRA_VXLAN_IF_VNI(zif)) {
		vni_info = VNI_INFO_FROM_ZEBRA_IF(zif);
		zebra_evpn_vl_vxl_ref(vni_info->vni.access_vlan, vni_info->vni.vni, zif);
