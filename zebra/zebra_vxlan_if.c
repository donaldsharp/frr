...
static int zebra_vxlan_if_add_vni(struct interface *ifp, struct zebra_vxlan_vni *vnip)
{
	...
	if (zl3vni) {
		...
		if (IS_ZEBRA_DEBUG_VXLAN)
			zlog_debug(
				"Add L3-VNI %u intf %s(%u) VLAN %u local IP %s master %u vrf %s is_l3svd %u",
				vni, ifp->name, ifp->ifindex, vnip->access_vlan,
				inet_ntoa(vxl->vtep_ip),
				zif->brslave_info.bridge_ifindex,
				vrf_id_to_name(zif->ifp->vrf_id),
...
int zebra_vxlan_if_vni_mcast_group_del(struct interface *ifp,
	vni_t vni_id, struct in_addr *mcast_group)
{
	struct zebra_if *zif = NULL;
	struct zebra_vxlan_vni *vni;
