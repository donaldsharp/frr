...
static int zebra_l2_bridge_if_vlan_walk_callback(struct hash_bucket *bucket, void *ctxt)
{
	int ret;
...
static void zebra_l2_bridge_if_vlan_iterate_callback(struct hash_bucket *bucket, void *ctxt)
{
        struct zebra_l2_bridge_vlan *bvlan;
	struct zebra_l2_bridge_if_ctx *ctx;
	...
	struct zebra_l2_bridge_vlan *bvlan, void *ctxt)
{
	...
	if (IS_ZEBRA_DEBUG_VXLAN)
		zlog_debug("access vlan %d bridge %s cleanup",
				bvlan->vid, zif->ifp->name);
...
static struct hash * zebra_l2_bridge_vlan_table_create(void)
{
	return hash_create(zebra_l2_bridge_vlan_hash_keymake,
		zebra_l2_bridge_vlan_hash_cmp, "Zebra L2 Bridge Vlan Table");
}
...
struct zebra_l2_bridge_vlan * zebra_l2_bridge_if_vlan_find(const struct zebra_if *zif, vlanid_t vid)
{
	...
	bvlan.vid = vid;
	bvl = (struct zebra_l2_bridge_vlan *)hash_lookup(br->vlan_table, (void *)&bvlan);
...
void zebra_l2_bridge_if_vlan_iterate(struct zebra_if *zif,
	int (*func)(struct zebra_if *zif, struct zebra_l2_bridge_vlan *, void *), void *arg)
{
	struct zebra_l2_bridge_if *br;
	struct zebra_l2_bridge_if_ctx ctx;
	...
	ctx.func = func;
	ctx.arg = arg;
	hash_iterate(br->vlan_table, zebra_l2_bridge_if_vlan_iterate_callback, &ctx);
}
