...
zebra_evpn_es_evi_find(struct zebra_evpn_es *es, zebra_vni_t *zvni)
{
	struct zebra_evpn_es_evi es_evi;
...
static void zebra_evpn_es_evi_show_entry_detail(
	struct vty *vty, struct zebra_evpn_es_evi *es_evi, json_object *json)
{
	char type_str[4];
...
static void zebra_evpn_es_evi_show_one_vni(zebra_vni_t *zvni, struct vty *vty,
						   ...
						   void *ctxt)
{
	...
	zebra_evpn_es_evi_show_one_vni(zvni, wctx->vty, wctx->json,
				       wctx->detail);
}
...
void zebra_evpn_vni_es_cleanup(zebra_vni_t *zvni)
{
	...
	RB_FOREACH_SAFE (es_evi, zebra_es_evi_rb_head, &zvni->es_evi_rb_tree,
			 es_evi_next) {
		zebra_evpn_local_es_evi_do_del(es_evi);
	}
...
void zebra_evpn_vl_vxl_ref(uint16_t vid, struct zebra_if *vxlan_zif)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("access vlan %d vni %u ref", acc_bd->vid,
			   vxlan_zif->l2info.vxl.vni);
...
void zebra_evpn_vl_vxl_deref(uint16_t vid, struct zebra_if *vxlan_zif)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("access vlan %d vni %u deref", acc_bd->vid,
			   vxlan_zif->l2info.vxl.vni);
...
void zebra_evpn_vxl_vni_set(struct zebra_if *zif, zebra_vni_t *zvni, bool set)
{
...
void zebra_evpn_vl_mbr_ref(uint16_t vid, struct zebra_if *zif)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("access vlan %d mbr %s ref", vid, zif->ifp->name);
...
void zebra_evpn_vl_mbr_deref(uint16_t vid, struct zebra_if *zif)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("access vlan %d mbr %s deref", vid, zif->ifp->name);
...
static void zebra_evpn_acc_vl_show_hash(struct hash_bucket *bucket, void *ctxt)
{
	...
	if (wctx->detail)
		zebra_evpn_acc_vl_show_entry_detail(wctx->vty, acc_bd,
						    wctx->json);
	else
		zebra_evpn_acc_vl_show_entry(wctx->vty, acc_bd, wctx->json);
}
...
static void zebra_evpn_nhg_update(struct zebra_evpn_es *es)
{
	...
	if (nh_cnt) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_NH) {
			...
			nh_str[0] = '\0';
			for (i = 0; i < nh_cnt; ++i)
				sprintf(nh_str + strlen(nh_str), "0x%x ",
					nh_ids[i].id);
			zlog_debug("es %s nhg 0x%x add %s", es->esi_str,
				   es->nhg_id, nh_str);
		}
		...
		if (es->flags & ZEBRA_EVPNES_NHG_ACTIVE) {
			if (IS_ZEBRA_DEBUG_EVPN_MH_NH)
				zlog_debug("es %s nhg 0x%x del", es->esi_str,
					   es->nhg_id);
			es->flags &= ~ZEBRA_EVPNES_NHG_ACTIVE;
...
static void zebra_evpn_nh_add(struct zebra_evpn_es_vtep *es_vtep)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_NH)
		zlog_debug("es %s vtep %s nh 0x%x add", es_vtep->es->esi_str,
...
static void zebra_evpn_nh_del(struct zebra_evpn_es_vtep *es_vtep)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_NH)
		zlog_debug("es %s vtep %s nh 0x%x del", es_vtep->es->esi_str,
			   inet_ntoa(es_vtep->vtep_ip), es_vtep->nh_id);
				   ...
				   struct in_addr vtep_ip)
{
	...
	if (!es_vtep) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("es %s vtep %s add", es->esi_str,
				   inet_ntoa(vtep_ip));
				   ...
				   struct in_addr vtep_ip)
{
	...
	if (es_vtep) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("es %s vtep %s del", es->esi_str,
				   inet_ntoa(vtep_ip));
...
static int zebra_evpn_es_send_add_to_client(struct zebra_evpn_es *es)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("send add local es %s %s to %s", es->esi_str,
					 ...
					 struct zebra_if *zif)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("local es %s add; nhg 0x%x if %s", es->esi_str,
	 ...
	 */
	zebra_evpn_es_local_mac_update(es, false /* force_clear_static */);
}
...
static void zebra_evpn_es_local_info_clear(struct zebra_evpn_es *es)
{
	 ...
	 */
	zebra_evpn_es_local_mac_update(es, true /* force_clear_static */);
...
static void zebra_evpn_local_es_del(struct zebra_evpn_es *es)
{
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES) {
		zif = es->zif;
		zlog_debug("local es %s del; nhg 0x%x if %s", es->esi_str,
...
static void zebra_evpn_remote_es_flush(struct zebra_evpn_es *es)
{
	...
	for (ALL_LIST_ELEMENTS(es->es_vtep_list, node, nnode, es_vtep)) {
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("es %s vtep %s flush", es->esi_str,
					    ...
					    zebra_vni_t *zvni, bool add)
{
	...
	zclient_create_header(
		s, add ? ZEBRA_LOCAL_ES_EVI_ADD : ZEBRA_LOCAL_ES_EVI_DEL,
	...
	if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
		zlog_debug("send %s local es %s evi %u to %s",
			   add ? "add" : "del", es->esi_str, zvni->vni,
			   zebra_route_string(client->proto));
					    ...
					    json_object *json)
{
	...
	if (json) {
		...
		vty_out(vty, " Ready for BGP: %s\n",
			(es->flags & ZEBRA_EVPNES_READY_FOR_BGP) ? "yes"
								 : "no");
		...
		for (ALL_LIST_ELEMENTS_RO(es->es_vtep_list, node, zvtep))
			vty_out(vty, "     %s nh: 0x%x\n",
				inet_ntoa(zvtep->vtep_ip), zvtep->nh_id);
...
void zebra_evpn_es_set_base_vni(zebra_vni_t *zvni)
{
	...
	if (zmh_info->es_base_vni) {
		...
		if (IS_ZEBRA_DEBUG_EVPN_MH_ES)
			zlog_debug("es base vni set to %d", zvni->vni);
