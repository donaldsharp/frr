...
int netlink_netconf_change(struct nlmsghdr *h, ns_id_t ns_id, int startup)
{
	...
	memset(tb, 0, sizeof(struct rtattr *) * NETCONFA_MAX + 1);
	zlog_debug("Received netconf_change");
...
int netconf_lookup_netlink(struct zebra_ns *zns)
{
	...
	ret = netlink_parse_info(netlink_netconf_change, netlink_cmd, &dp_info, 0, 1);
	return ret;
}
