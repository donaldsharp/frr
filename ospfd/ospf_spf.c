...
static void ospf_orr_spf_calculate_schedule_worker(struct event *thread)
{
	afi_t afi;
	safi_t safi;
	struct ospf_area *area;
	...
	FOREACH_AFI_SAFI (afi, safi) {
		...
		for (ALL_LIST_ELEMENTS_RO(orr_root_list, rnode, root)) {
			if (!root || !root->router_lsa_rcvd)
				continue;
			ospf_orr_debug("%s: For %s %s, MPLS TE Router address %pI4 advertised by %pI4",
			...
			ospf_orr_debug("%s: ospf install new route, vrf %s id %u new_table count %lu",
				       __func__,
