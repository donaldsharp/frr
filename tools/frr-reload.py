...
def bgp_delete_inst_move_line(lines_to_del):
    ...
    for (ctx_keys, line) in lines_to_del:
        ...
        if (
            ...
            bgp_default_inst = True
        # Find bgp vrf inst
        elif ctx_keys[0].startswith("router bgp") and not line and "vrf" in ctx_keys[0]:
            bgp_vrf_inst = True

    if bgp_default_inst and bgp_vrf_inst:
        for (ctx_keys, line) in lines_to_del:
...
def ignore_delete_re_add_lines(lines_to_add, lines_to_del):
    ...
    for (ctx_keys, line) in lines_to_del:
                        ...
                        if ctx_keys_al[0].startswith("router bgp"):
                            ...
                            if rm_match:
                                ...
                                if len(ctx_keys_al) == 1:
                                    ...
                                    if len(ctx_keys) > 1 and len(ctx_keys_al) > 1 and ctx_keys[1] == ctx_keys_al[1]:
                                        lines_to_del_to_del.append((ctx_keys_al, line))

                    if adjust_for_bgp_node == 1:
                        for (ctx_keys_dl, dl_line) in lines_to_del:
                            if ctx_keys_dl[0].startswith("router bgp") and len(ctx_keys_dl) > 1 and ctx_keys_dl[1] == "address-family ipv4 unicast":
        ...
        '''
        re_acl_pfxlst = re.search('^(ip |ipv6 |)(prefix-list|access-list)(\s+\S+\s+)(seq \d+\s+)(permit|deny)(.*)$',
                                  ctx_keys[0])
        if re_acl_pfxlst:
            found = False
            tmpline = (re_acl_pfxlst.group(1) + re_acl_pfxlst.group(2) +
                       re_acl_pfxlst.group(3) + re_acl_pfxlst.group(5) +
