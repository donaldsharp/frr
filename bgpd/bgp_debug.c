...
DEFPY(debug_bgp_optimal_route_reflection, debug_bgp_optimal_route_reflection_cmd,
      "[no$no] debug bgp optimal-route-reflection",
      NO_STR DEBUG_STR BGP_STR BGP_ORR_DEBUG)
{
	if (vty->node == CONFIG_NODE) {
		if (no)
	    ...
	    BGP_STR)
{
	...
	if (BGP_DEBUG(optimal_route_reflection, ORR))
		vty_out(vty, "  BGP Optimal Route Reflection debugging is on\n");
