...
#undef COMMUNITY_VAL_STR
#define COMMUNITY_VAL_STR "Community number in aa:nn format or internet|local-AS|no-advertise|no-export or additive\n"
...
DEFPY_YANG(
	...
	"no bgp extcommunity-list <(1-99)$num|standard EXTCOMMUNITY_LIST_NAME$name> [seq (1-4294967295)$seq] <deny|permit>$action <rt|soo>$type AA:NN...",
	NO_STR
	BGP_STR
	EXTCOMMUNITY_LIST_STR
	"Extcommunity list number (standard)\n"
...
/*
 * cli_show callbacks for frr-bgp-filter YANG module
 * These are used by mgmtd to generate 'show running-config' output.
 */

static void bgp_community_list_entry_cli_show(struct vty *vty,
					      ...
					      bool show_defaults)
{
	...
	int64_t seq = yang_dnode_get_uint32(dnode, "sequence");

	if (strcmp(type, "community-list-standard") == 0) {
		const struct lyd_node *community;

		community = yang_dnode_get(dnode, "standard-community-string");
		if (!community)
			return;

		vty_out(vty, "bgp community-list standard %s seq %" PRId64 " %s",
			name, seq, action);

		LY_LIST_FOR(community, community) {
			vty_out(vty, " %s", lyd_get_value(community));
		}
		vty_out(vty, "\n");
	} else {
		/* expanded */
		vty_out(vty, "bgp community-list expanded %s seq %" PRId64 " %s %s\n",
			name, seq, action,
			yang_dnode_get_string(dnode, "expanded-community-string"));
	}
}

static void bgp_lcommunity_list_entry_cli_show(struct vty *vty,
					       const struct lyd_node *dnode,
					       bool show_defaults)
{
	const char *name = yang_dnode_get_string(dnode, "../name");
	...
	if (strstr(type, "standard")) {
		const struct lyd_node *lcomm;

		/* Skip output if community values not yet set — the NB
		 * framework calls cli_show for each child change, so we
		 * may be called before the leaf-list is populated.
		 */
		lcomm = yang_dnode_get(dnode, "standard-large-community-string");
		if (!lcomm)
			return;

		vty_out(vty, "bgp large-community-list standard %s seq %" PRId64 " %s",
			name, seq, action);

		LY_LIST_FOR(lcomm, lcomm) {
			vty_out(vty, " %s", lyd_get_value(lcomm));
		}
		vty_out(vty, "\n");
	} else {
		vty_out(vty, "bgp large-community-list expanded %s seq %" PRId64 " %s %s\n",
			name, seq, action,
			yang_dnode_get_string(dnode, "expanded-large-community-string"));
	}
}

static void bgp_extcommunity_list_entry_cli_show(struct vty *vty,
						 const struct lyd_node *dnode,
						 bool show_defaults)
{
	const char *name = yang_dnode_get_string(dnode, "../name");
	...
	if (strstr(type, "standard")) {
		const struct lyd_node *ecomm;

		/* Check for rt values */
		ecomm = yang_dnode_get(dnode, "extcommunity-rt");
		if (ecomm) {
			vty_out(vty, "bgp extcommunity-list standard %s seq %" PRId64 " %s rt",
				name, seq, action);
			LY_LIST_FOR(ecomm, ecomm) {
				vty_out(vty, " %s", lyd_get_value(ecomm));
			}
			vty_out(vty, "\n");
			return;
		}

		/* Check for soo values */
		ecomm = yang_dnode_get(dnode, "extcommunity-soo");
		if (ecomm) {
			vty_out(vty, "bgp extcommunity-list standard %s seq %" PRId64 " %s soo",
				name, seq, action);
			LY_LIST_FOR(ecomm, ecomm) {
				vty_out(vty, " %s", lyd_get_value(ecomm));
			}
		...
		/* Check for nt (node-target) values */
		ecomm = yang_dnode_get(dnode, "extcommunity-nt");
		if (ecomm) {
			vty_out(vty, "bgp extcommunity-list standard %s seq %" PRId64 " %s nt",
				name, seq, action);
			LY_LIST_FOR(ecomm, ecomm) {
				vty_out(vty, " %s", lyd_get_value(ecomm));
			}
		...
		vty_out(vty, "bgp extcommunity-list expanded %s seq %" PRId64 " %s %s\n",
			name, seq, action,
			yang_dnode_get_string(dnode, "expanded-extcommunity-string"));
	}
}

static void bgp_as_path_list_entry_cli_show(struct vty *vty,
					    const struct lyd_node *dnode,
					    bool show_defaults)
{
	const char *name = yang_dnode_get_string(dnode, "../name");
	const char *action = yang_dnode_get_string(dnode, "action");
	int64_t seq = yang_dnode_get_uint32(dnode, "sequence");
	const char *regex = yang_dnode_get_string(dnode, "as-path-regex");

	vty_out(vty, "bgp as-path access-list %s seq %" PRId64 " %s %s\n",
		name, seq, action, regex);
}

/* YANG module info for frr-bgp-filter */
const struct frr_yang_module_info frr_bgp_filter_cli_info = {
	.name = "frr-bgp-filter",
	.ignore_cfg_cbs = true,
	.nodes = {
		{
			.xpath = "/frr-filter:lib/frr-bgp-filter:community-list/entry",
			.cbs.cli_show = bgp_community_list_entry_cli_show,
		},
		{
			.xpath = "/frr-filter:lib/frr-bgp-filter:large-community-list/entry",
			.cbs.cli_show = bgp_lcommunity_list_entry_cli_show,
		},
		{
			.xpath = "/frr-filter:lib/frr-bgp-filter:extcommunity-list/entry",
			.cbs.cli_show = bgp_extcommunity_list_entry_cli_show,
		},
		{
			.xpath = "/frr-filter:lib/frr-bgp-filter:as-path-list/entry",
			.cbs.cli_show = bgp_as_path_list_entry_cli_show,
		},
		{
			.xpath = NULL,
		},
