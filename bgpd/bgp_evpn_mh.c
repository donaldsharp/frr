	...
	*route_changed = 1;
	...
	if (remote_pi) {
		flog_err(
			...
			bgp->vrf_id, es->esi_str, inet_ntoa(es->originator_ip));
		return -1;
	}
	...
	if (*route_changed) {
		if (BGP_DEBUG(evpn_mh, EVPN_MH_RT))
			zlog_debug(
				"local ES %s vni %u route-type %s nexthop %s updated",
					       ...
					       struct attr *attr)
{
	...
	attr->ecommunity = ecommunity_merge(attr->ecommunity, &ecom_es_rt);
						  ...
						  struct attr *attr)
{
	...
	for (ALL_LIST_ELEMENTS_RO(es->es_evi_list, evi_node, es_evi)) {
		if (!CHECK_FLAG(es_evi->flags, BGP_EVPNES_EVI_LOCAL))
			continue;
		for (ALL_LIST_ELEMENTS_RO(es_evi->vpn->export_rtl, rt_node,
					  ecom))
			attr->ecommunity =
				       ...
				       struct prefix_evpn *p)
{
	...
	if (vpn) {
		...
		/* Create or update route entry. */
		ret = bgp_evpn_mh_route_update(bgp, es, vpn, afi, safi, rn,
	...
	if (route_changed) {
		...
		rn = bgp_global_evpn_node_get(bgp->rib[afi][safi], afi, safi, p,
					      global_rd);
		bgp_evpn_mh_route_update(bgp, es, vpn, afi, safi, rn, attr_new,
					 ...
					 bool add)
{
	...
	if (!IS_BGP_INST_KNOWN_TO_ZEBRA(bgp)) {
		if (BGP_DEBUG(zebra, ZEBRA))
			zlog_debug(
				"No zebra instance, not installing remote es %s",
				es->esi_str);
		return 0;
	}
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("Tx %s Remote ESI %s VTEP %s", add ? "ADD" : "DEL",
			   es->esi_str, inet_ntoa(es_vtep->vtep_ip));
				    ...
				    struct bgp_evpn_es_vtep *es_vtep, bool esr)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("es %s vtep %s del %s", es_vtep->es->esi_str,
			   inet_ntoa(es_vtep->vtep_ip), esr ? "esr" : "ead");
	if (esr) {
...
void bgp_evpn_path_es_unlink(struct bgp_path_es_info *es_info)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_RT))
		zlog_debug(
			"vni %u path %s unlinked from es %s", es_info->vni,
...
void bgp_evpn_path_es_link(struct bgp_path_info *pi, vni_t vni, esi_t *esi)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_RT))
		zlog_debug(
			"vni %u %p path %s linked to es %s", vni, pi,
			prefix2str(&pi->net->p, prefix_buf, sizeof(prefix_buf)),
			es->esi_str);
...
int bgp_evpn_local_es_del(struct bgp *bgp, esi_t *esi)
{
	...
	es = bgp_evpn_es_find(esi);
	if (!es) {
		flog_warn(EC_BGP_EVPN_ESI, "%u: ES %s missing at local ES DEL",
			  bgp->vrf_id, es->esi_str);
		return -1;
	}
			  ...
			  uint16_t df_pref)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("add local es %s orig-ip %s df_pref %u", es->esi_str,
				   ...
				   json_object *json)
{
	...
	if (json) {
		...
		vty_out(vty, "%-30s %-5s %-21s %-8d %s\n", es->esi_str,
			type_str, buf1, listcount(es->es_evi_list), vtep_str);
	}
}
					      ...
					      bool v4_nhg)
{
	...
	for (ALL_LIST_ELEMENTS_RO(es->es_vtep_list, node, es_vtep)) {
		...
		++nh_cnt;
		if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
			zlog_debug("nhg %d vtep %s l3-svi %d", nhg_id,
				   inet_ntoa(es_vtep->vtep_ip),
		...
		struct bgp *bgp_vrf)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("es %s vrf %u nhg %d v6_nhg %d create", es->esi_str,
...
static void bgp_evpn_es_vrf_delete(struct bgp_evpn_es_vrf *es_vrf)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug("es %s vrf %u nhg %d delete", es->esi_str,
				    ...
				    struct bgp_evpn_es_evi_vtep *evi_vtep)
{
	 ...
	 */
	if ((evi_vtep->flags & BGP_EVPN_EVI_VTEP_EAD) == BGP_EVPN_EVI_VTEP_EAD)
	...
	if (bgp) {
		...
		if (CHECK_FLAG(es->flags, BGP_EVPNES_ADV_EVI)) {
			build_evpn_type1_prefix(&p, BGP_EVPN_AD_EVI_ETH_TAG,
						&es->esi, es->originator_ip);
			if (bgp_evpn_type1_evi_route_delete(bgp, es,
							    es_evi->vpn, &p))
				flog_err(
			       ...
			       const struct prefix_evpn *p)
{
	...
	if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
		zlog_debug(
			"del remote %s es %s evi %u vtep %s",
			p->prefix.ead_addr.eth_tag ? "ead-es" : "ead-evi",
			esi_to_str(&p->prefix.ead_addr.esi, buf, sizeof(buf)),
			vpn->vni, inet_ntoa(p->prefix.ead_addr.ip.ipaddr_v4));
				       ...
				       struct bgp_evpn_es_evi *es_evi)
{
	...
	for (ALL_LIST_ELEMENTS_RO(es_evi->es_evi_vtep_list, node, evi_vtep)) {
		...
		if (first) {
			first = false;
			sprintf(vtep_str + strlen(vtep_str), "%s(%s)",
				inet_ntoa(evi_vtep->vtep_ip), vtep_flag_str);
		} else {
			sprintf(vtep_str + strlen(vtep_str), ",%s(%s)",
				inet_ntoa(evi_vtep->vtep_ip), vtep_flag_str);
		}
...
static void bgp_evpn_es_evi_set_inconsistent(struct bgp_evpn_es_evi *es_evi)
{
	if (!CHECK_FLAG(es_evi->flags, BGP_EVPNES_EVI_INCONS_VTEP_LIST)) {
		if (BGP_DEBUG(evpn_mh, EVPN_MH_ES))
			zlog_debug(
				"inconsistency detected - es %s evi %u vtep list mismatch",
				es_evi->es->esi_str, es_evi->vpn->vni);
		...
		if (!es_evi->es->incons_evi_vtep_cnt
			...
			zlog_debug(
				"inconsistency detected - es %s vtep list mismatch",
				es_evi->es->esi_str);
...
static uint32_t bgp_evpn_es_run_consistency_checks(struct bgp_evpn_es *es)
{
	...
	for (ALL_LIST_ELEMENTS_RO(es->es_evi_list, evi_node, es_evi)) {
		...
		while ((es_vtep = bgp_evpn_es_get_next_active_vtep(es,
								   es_vtep))) {
			evi_vtep = bgp_evpn_es_evi_get_next_active_vtep(
				es_evi, evi_vtep);
			if (!evi_vtep) {
				bgp_evpn_es_evi_set_inconsistent(es_evi);
				break;
