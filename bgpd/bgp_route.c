		...
		for (; pi; pi = pi->next) {
			...
			if (!brief) {
				if (type == bgp_show_type_prefix_version) {
					uint32_t version = strtoul(output_arg,
								   NULL, 10);
				...
				if (type == bgp_show_type_community_alias) {
					...
					if (picomm) {
						frrstr_split(picomm->str, " ",
							     &communities, &num);
						for (int i = 0; i < num; i++) {
							const char *com2alias = bgp_community2alias(
								communities[i]);
							if (!found &&
							    strcmp(alias,
								   com2alias) ==
...
static void bgp_prefix_json_info_add(struct vty *vty, json_object *json_flags,
				     ...
				     int prefix_path_count, int multi_path_count)
{
		...
		if (type == bgp_show_adj_route_received) {
			for (ain = dest->adj_in; ain; ain = ain->next) {
				...
				bgp_fib_flags_info(vty, bgp, dest, json_flags,
						   best_path_selected);
				bgp_prefix_json_info_add(vty, json_flags,
							 json_info,
							 prefix_path_count,
							 multi_path_count);
